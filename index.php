...
$sql = "SELECT * FROM productos WHERE id NOT IN (SELECT id_producto FROM carrito WHERE id_usuario = $_SESSION[id])";
        ...
        <?php while ($row  = $result->fetch_assoc()) { ?>
            <div class="card">
                ...
                <div class="acciones">
                    ...
                    <p><?= "$" . number_format($row['precio'], 2) ?></p> <br><br>
                    <button onclick="addProduct(this.id)" id="<?= $row['id'] ?>">Añadir al carrito</button>
                </div>
            </div>
        <?php } ?>
    </div>
